...
public class FolderCommandHandler implements HttpRequestHandler {
	private static final String EXTERNAL_STORAGE_PATH = Environment
	...
	public void handle(HttpRequest request, HttpResponse response,
		...
		boolean isListDirectory = pref.getBoolean(
				Constants.PREF_DIRECTORY_LISTING, true);
	...
	private HttpEntity getEntityFromUri(String uri, HttpResponse response) {
		...
		AppLog.logInfo("URI: " + uri);
		AppLog.logInfo(filepath);
		...
		if (uri.equalsIgnoreCase("/") || uri.length() <= 0) {
			filepath = EXTERNAL_STORAGE_PATH + "/"
					+ pref.getString(Constants.PREF_DIRECTORY, "");
		} else {
		...
		if (file.isDirectory()) {
			...
			try {
				entity = new StringEntity(Utility.openHTMLString(context,
						R.raw.notfound));
		...
		AppLog.logInfo("Content Type: " + contentType);
	...
	private String getDirListingHTML(File file) {
		...
		for (String fileName : fileList) {
			...
			html += "<tr>";
			html += "<td><a href='http://" + Utility.getLocalIpAddress() + ":"
					+ serverPort + "/dir/" + fileinfo[0] + "'>" + fileinfo[0]
					+ "</a><br></td>";
	...
	private ArrayList<String> getDirListing(File file) {
		...
		for (File f : fileArrayList) {
			fileList.add(f.getAbsolutePath().substring(
					EXTERNAL_STORAGE_PATH.length() + 1)
