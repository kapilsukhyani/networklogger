...
import com.enlighten.transparentproxy.app.AppSettings;
...
public class AWSActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Thread.setDefaultUncaughtExceptionHandler(new UncaughtExceptionHandler() {
			...
			public void uncaughtException(Thread thread, Throwable ex) {
				ex.printStackTrace();
				System.out.println("In global exception handler " + ex);
			}
		});
		prepareViews();
		setButtonHandlers();
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menuPreference: {
			startActivityForResult(
					new Intent(this, AWSPreferenceActivity.class),
	...
	private void setButtonText(boolean isServiceRunning) {
		((Button) findViewById(R.id.btnStartStop))
				.setText(getString(isServiceRunning ? R.string.stop_caption
						: R.string.start_caption));
	}
	...
	private void setInfoText(boolean isServiceRunning) {
		...
		if (isServiceRunning) {
			...
			text = getString(R.string.log_running)
					...
					+ ":"
					+ pref.getString(Constants.PREF_SERVER_PORT, ""
